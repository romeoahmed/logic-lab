...
The native V1 project carrier is one `.logiclab` ZIP profile containing a strict JSON Project Document and canonical binary memory images. Project Format owns bounded spooling, ZIP validation, strict DTOs, migrations, digests, and encoding, and returns a complete Import Candidate. Application asks Project Editor to create Project Genesis, compiles that first Project Revision, and publishes a new Workspace only when both steps succeed.
