...
|---|---|---|---|
| authoring, Compilation, Simulation, analysis, format, diagnostics, symbols | closed V1 behavior and Module interfaces | immutable lineage; the closed V1 Edit Intent catalog with atomic public-Port and instance-contract migration, deletion, Memory Images, Symbol Profile/Variant, and ordered Annotations; explicit Net/Junction/Wire Geometry editing; Domain-resolved generated split/concat and arithmetic Ports; zero/sign extension, constant-source, unsigned comparison, full-adder/subtractor, and logical-shift execution; bounded unknown shift-case settlement; hierarchical combinational Compilation with complete occurrence provenance and recursion evidence; cyclic combinational Least Information Fixed Point settlement with synchronous-oracle and fair-worklist-order differential, Indeterminate Feedback, contention, defect, exhaustion, and epoch-reset evidence; the complete sequential family with Clock Source, latches, flip-flops, register, shift register, counter, unknown controls, derived clocks, exact-state Zero-time Oscillation, causal Trigger Batches, storage normalization, and atomic rollback; explicit ROM and single-port RAM images, asynchronous reads, conservative unknown addressing and write controls, definite rising-edge Trigger Batch writes, occurrence-local memory state, bounded allocation, and atomic RAM rollback; atomic Simulation Session, diagnostics, scalar oracle, and packed differential path | Boolean Analysis, Project Format, TeachingMixed geometry generation, and their conformance suites |
...
Verification snapshot (2026-08-04): locked restore, formatting, warning-clean build, whitespace validation, and the whole-solution TUnit suite passed. The executable evidence above covers the completed vertical slices; it does not qualify the application for release while any remaining-evidence row is open.
