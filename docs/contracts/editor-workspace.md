...
does not take the unpublished staging, and leaves any earlier ticket for the
...
The Workspace mutation and idempotency result share one in-memory commit section. A Durable Save stores its idempotency receipt and current-revision pointer in one repository transaction.

Claim stores the originating Workspace ID as an immutable unique claim key in the same transaction as the new Durable Project and receipt. That durable fence outlives receipt pruning, Client Intent IDs, and attachment generations. A later authorized Claim for the same Workspace recovers the existing Durable Project and records the new receipt instead of creating another row; an ownership mismatch remains indistinguishable from absence.
...
`CloseWorkspace` remains available after an uncertain commit closes the idempotency
window because it performs only authorized in-memory retirement and cannot replay the
repository write. It is absence-idempotent: closing an already absent or expired
Workspace returns `WorkspaceClosed` without recreating state. Other commands against
