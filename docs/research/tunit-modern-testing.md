...
- put future bUnit projects in TUnit reflection mode, because generated Razor test cases are the documented reason for that mode.
...
**Source fact:** TUnit documents reflection mode specifically for tests generated by another source generator, including Razor components in bUnit. For a project exclusively in reflection mode, `[assembly: ReflectionMode]` can be paired with `EnableTUnitSourceGeneration=false` to avoid unused generator cost ([engine modes](https://tunit.dev/docs/execution/engine-modes#reflection-mode)).

**Logic Lab inference:** When the planned bUnit project appears, isolate it as a separate test executable, select assembly reflection mode there, and qualify `EnableTUnitSourceGeneration=false`. Do not force Domain, Engine, or future non-Razor test projects into reflection mode. Future Playwright tests can use the official `TUnit.Playwright` integration without changing semantic ownership ([Playwright integration](https://tunit.dev/docs/examples/playwright)).
...
| future bUnit test | separate TUnit reflection-mode project | documented cross-source-generator compatibility |
