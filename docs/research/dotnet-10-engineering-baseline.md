...
Before migration, executable slices `01` and `02` used xUnit v3 and passed 140 tests. On 2026-07-31 both test projects, locks, runner files, attributes, awaited assertions, fixed data, and property entry points migrated atomically to the selected TUnit stack. Discovery now reports 252 tests: the original semantic evidence plus an independently reported 112-case word-tail matrix. The official TUnit xUnit migration analyzer can automate common syntax, but this migration was reviewed manually for data-source types, collection ordering, assertion argument order, property shrinking/replay, and parallel behavior ([xUnit migration](https://tunit.dev/docs/migration/xunit/)).
