...
- Circuit loss freezes the last acknowledged semantic display and allows browser-local pan/zoom only. Web replaces the current history entry with the opaque Workspace locator and a versioned attachment fence in browser history state; the fence never appears in the URL. A surviving circuit or full document reload reattaches with that prior fence and replaces the state with the returned generation. If a fresh circuit has no valid fence, Web first attempts `InitialAttach`; `stale_workspace_attachment` permits one `RecoverAttach` for an authenticated Durable Project, reauthorizing the caller and fencing the displaced generation. Sandbox recovery never bypasses possession of its prior fence. Every success returns a complete Workspace Projection and never resumes Run automatically. This explicit browser adapter state is required because Blazor circuit state itself does not survive a full-page refresh ([server-side state management](https://learn.microsoft.com/en-us/aspnet/core/blazor/state-management/server?view=aspnetcore-10.0#circuit-state-persistence)).
- A rejected reattachment or recovery because server state was lost offers `Reload workspace` or another explicitly authorized recovery flow. A browser reconnection indicator never claims that a Workspace command committed.
...
The host applies separate bounded policies to request rate, request body bytes, concurrent transfers, staged export count/bytes, expanded package work, Workspace admission, and background work. ASP.NET Core rate limiting is an ingress control, not a complete DDoS defense or a replacement for Module policy; official guidance requires load testing configured policies ([rate limiting](https://learn.microsoft.com/en-us/aspnet/core/performance/rate-limit?view=aspnetcore-10.0)). Export download admission composes a global concurrency limiter with a fixed window partitioned by the typed Workspace caller; the global limiter runs first, both queue zero requests, and both run before ticket redemption. Package preparation independently admits at most four concurrent writers with no queue before any staging object or expanded package work is created; exhaustion returns `export_capacity_unavailable`. The initial provisional defaults are four concurrent package preparations, eight concurrent downloads, twenty download attempts per minute per caller, 128 published staging objects, and 512 MiB of published carrier bytes. Operators configure these limits together under `LogicLab:ProjectExports`; load evidence must calibrate them before they become measured acceptance thresholds.
