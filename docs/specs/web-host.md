...
- Circuit loss freezes the last acknowledged semantic display and allows browser-local pan/zoom only. A surviving circuit reattaches with its prior fence. After a full document reload creates a fresh circuit without that fence, Web first attempts `InitialAttach`; `stale_workspace_attachment` permits one `RecoverAttach` that reauthorizes the current caller and fences the displaced generation. Either success returns a complete Workspace Projection and never resumes Run automatically. This explicit recovery is required because Blazor circuit state does not survive a full-page refresh ([server-side state management](https://learn.microsoft.com/en-us/aspnet/core/blazor/state-management/server?view=aspnetcore-10.0#circuit-state-persistence)).
