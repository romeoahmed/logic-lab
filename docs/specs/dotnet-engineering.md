...
The repository currently has executable xUnit v3 evidence for slices `01` and `02`. This documentation-first decision is intentionally ahead of the project files: the next test-changing implementation slice migrates both existing test projects, centrally pinned packages, lock files, runner configuration, assertions, and property tests as one atomic change. No project mixes xUnit and TUnit after that slice, and no new xUnit test is added in the interim. The migration target and rationale are grounded in [TUnit Modern Testing Research](../research/tunit-modern-testing.md).
