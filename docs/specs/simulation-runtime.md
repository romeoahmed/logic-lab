...
The working layer includes Driver and Net values, component state, memory writes, event-frontier state, reusable cyclic-settlement scratch, one serial evaluator's temporary materialization envelope, diagnostics, and staged Trace transitions. Later Delta Steps can see earlier working commits; callers cannot.
