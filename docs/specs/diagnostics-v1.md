...
## 9. Diagram Presentation diagnostics
...
| Workspace               | `authentication_required`, `workspace_not_found`, `workspace_expired`, `stale_workspace_attachment`, `idempotency_key_conflict`, `idempotency_window_expired`, `durable_claim_unresolved`, `durable_display_name_invalid`, `project_revision_precondition_failed`, `projection_version_precondition_failed`, `compilation_generation_unavailable`, `session_precondition_failed`, `run_generation_precondition_failed`, `durable_save_conflict`, `build_fingerprint_mismatch`, `hot_swap_incompatible`, `workspace_admission_rejected`, `workspace_cancelled`, `workspace_infrastructure_failure`, `workspace_internal_defect`, `export_capacity_unavailable`, `export_expired` |
