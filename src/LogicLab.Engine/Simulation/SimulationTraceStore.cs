        ...
        CancellationToken cancellationToken)
    {
            ...
            && (UInt128)transitions[transitionIndex].LogicalTime == range.StartInclusive)
        {
            ...
            hadTransition |= firstTransition is not null
                && !firstTransition.Value.ContentEquals(transition.Value, cancellationToken);
            ...
            && (UInt128)transitions[transitionIndex].LogicalTime < range.EndExclusive)
        {
            cancellationToken.ThrowIfCancellationRequested();
            var transition = transitions[transitionIndex++];
            if (!lastValue.ContentEquals(transition.Value, cancellationToken))
            {
                hadTransition = true;
                lastValue = transition.Value;
                hadMixedValues |= !firstValue.ContentEquals(lastValue, cancellationToken);
            }
