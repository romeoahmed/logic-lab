...
internal sealed partial class WorkbenchComponentTests
{
    ...
    {
        await using var context = CreateContext();
        await using var workspace = new DelegatingEditorWorkspace();
    ...
    [Test]
    public async Task Editor_ImportedWorkspaceFails_PropagatesErrorWithoutReportingUploadLimit()
    {
        await using var context = CreateContext();
        await using var workspace = new FailingImportWorkspace();
        var rendered = RenderEditor(context, workspace);
        _ = await rendered.WaitForElementAsync("[data-command='import']:not([disabled])");
        var package = await CreatePackageAsync();
        ...
        var observed = await Assert.That(() =>
            rendered.FindComponent<InputFile>().UploadFiles(
                InputFileContent.CreateFromBinary(package, "project.logiclab")))
            .ThrowsExactly<IOException>();

        await Assert.That(observed).IsSameReferenceAs(workspace.Failure);
    }
